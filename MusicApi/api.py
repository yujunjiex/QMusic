...
class Parse(object):
    ...
    def song_url(cls, song):
        if "url" in song:
            ...
            url = song["url"]
            if url is None:
                new_url = NetEase().songs_url([song["id"]])[0]["url"]
                if new_url is None:     # 处理部分歌曲版权问题
                    return None, "LD 128k"
                # return Parse._song_url_by_id(song["id"])
                return new_url, "LD 128k"
            br = song["br"]
            if br >= 320000:
                quality = "HD"
            elif br >= 192000:
            ...
            # songs_detail resp
            # return Parse._song_url_by_id(song["id"])
            return NetEase().songs_url([song["id"]])[0]["url"], "LD 128k"
    ...
    def songs(cls, songs):
        ...
        for song in songs:
            ...
            # url, quality = Parse.song_url(song)     # 可以考虑先不给URL，在列表双击时根据id去申请url
            # if not url:     # url为None的歌曲信息不进行返回
            #     continue
        ...
        return [
            {
                ...
                "cover_url": pl["picUrl"] if "picUrl" in pl else pl["coverImgUrl"]
            }
            for pl in playlists
        ]
...
class NetEase(object):
    ...
    def album_info(self, album_id):
        path = "/weapi/v1/album/{}".format(album_id)
        return self.request("POST", path).get("album", [])
    ...
    def dig_info(self, data, dig_type):
        ...
        if dig_type == "songs" or dig_type == "fmsongs":
            urls = self.songs_detail([s["id"] for s in data])
            # timestamp = time.time()
            ...
            for s in data:
                ...
                if url_index is None:
                    ...
                    continue
                s["url"] = None         # urls[url_index]["url"]
                s["br"] = 128000    # urls[url_index]["br"] (此处暂时默认低mp3音质)
                # s["expires"] = urls[url_index]["expi"]
                s["long"] = self.convert_interval_format(urls[url_index]["dt"])
